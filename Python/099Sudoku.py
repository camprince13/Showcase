...
def cellContains (cell, num):
    #if any(str(num) in s for s in cell):
...
def totalBlankSquares():
    ...
    while ink <= 9:
        ...
        ink += 1
    #print('There are ' + str(SudLines.count('0')) + ' blank cells left')
...
def checkCellAboveBelow(currentCell, num):
    ...
    #output += cellContains(tmpCell, num)
...
def solve(currentPos, currentCell):#, checkNum
    ...
    elif countBlankSquares(retSingleCell(currentCell)) == 1:# and retNumAtPos(currentCell, currentPos) == '0':
        tmpCell = retSingleCell(currentCell)
        tmpNmList = ['1', '2', '3', '4', '5', '6', '7', '8', '9']
        ink = 1
        while cellContains(tmpCell, 0) == 'true': #and currentPos <= 9:
...
##
                ...
                #elif checkCellAboveBelow(currentCell, checkNum) == 2 and checkCellLeftRight(currentCell, checkNum) == 1 and checkColumn(currentPos, currentCell, checkNum) == 0 and checkRow(currentPos, currentCell, checkNum) == 0:
                #elif twoUD1LRMultAvail(currentCell, currentPos, checkNum) == 'true' and checkCellsCol(currentPos, currentCell, 0) == 2: #shorter replacement
				#even shorter
                ...
                # elif checkCellLeftRight(currentCell, checkNum) == 2:
                #elif checkCellLeftRight(currentCell, checkNum) == 2 and checkCellAboveBelow(currentCell, checkNum) == 1 and checkRow(currentPos, currentCell, checkNum) == 0 and checkColumn(currentPos, currentCell, checkNum) == 0:
                #elif twoLR1UDMultAvail (currentCell, currentPos, checkNum) == 'true' and checkCellsRow(currentPos, currentCell, 0) == 2: #shorter replacement
                #even shorter # 2
                ...
                #if 1 vert and 1 available
                #elif checkCellAboveBelow(currentCell, checkNum) == 1 and checkRow(currentPos, currentCell, checkNum) == 0 and checkColumn(currentPos, currentCell, checkNum) == 0:
                #    if checkCellsCol(currentPos, currentCell, 0) == 1 and checkCellsRow(currentPos, currentCell, 0) == 1:
                #        tmpCell = retSingleCell(currentCell)
                #        tmpCell[(currentPos - 1)] = str(checkNum)
                #        CellEdit(tmpCell, currentCell)
                #        checkNum = 10
                ...
                #if 1 horizontal and 1 available
                #elif checkCellLeftRight(currentCell, checkNum) == 1 and checkRow(currentPos, currentCell, checkNum) == 0 and checkColumn(currentPos, currentCell, checkNum) == 0:
    ...
    arr = [currentPos, currentCell]#, checkNum
        
            
        #if on
        
        #try to solve square
        #if cell(s) above/below have num
        #if cell(s) left/right have num
        #if cellContains(retSingleCell(currentCell), checkNum) == 'false':
            #[1,2,3,4,5,6,879].remove    if count ==1  
        #i = 1
    global stale
    stale += 1
...
#testEdit(Char0, 1, 5, 1)

#print(retSingleCell(1))
#print(retSingleCell(2))
#print(retNumAtPos(1, 8))
totalBlankSquares()

#print(checkCellLeftRight(1, 1))

i = 1
current = [1, 1]; #current pos, current cell, checkNum
#while i <= 1000:
while stale <= 10000 and solved == 'false':

    current = solve(current[0], current[1])#, current[2]
    #print(i)
    i += 1
